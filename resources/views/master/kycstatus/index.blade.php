    ...
    <div class="w-full overflow-hidden rounded-lg shadow-md">
        <div class="flex flex-wrap items-center px-4 py-2">
            <div class="relative w-full max-w-full flex-grow flex-1">
                <h3 class="font-semibold text-base text-gray-900 dark:text-gray-50">KYC Status  Details</h3>
            </div>
            ...
            <div class="relative w-full max-w-full flex-grow flex-1 text-right">
                <!-- <a href="{{ route('master.service.add') }}" class="action-btn py-1.5 font-light text-sm px-4 inline-block mt-2 rounded-lg text-center shadow-md bg-blue-500 text-white  transition duration-200 repairsandkycstatusadd" type="button" data-toggle="modal" data-target="#repairsandkycstatusModal">Add Service</a> -->
            </div>
        </div>
                ...
                <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                    ...
                    @foreach($kycstatus as $kycstatuss)
                        <tr class="text-sm bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-700 dark:text-gray-400 transition duration-200">
                            <td class="px-4 py-3">{{ $kycstatuss->id }}</td>
                            <td class="px-4 py-3">{{ $kycstatuss->status }}</td>
                            <?php
                            $id = $kycstatuss->id;
                            ?>
                                <td class="px-4 py-3">
...
<script type="text/javascript">
    ...
    $(document).on('click', '.repairsandkycstatusedit', function() {
        ...
        $('#repairsandkycstatusModal').modal('show');
    });

    $(document).on('click', '.close', function() {
        $('#repairsandkycstatusModal').modal('hide');
    });
    $(document).on('click', '.closebutton', function() {
        $('#repairsandkycstatusModal').modal('hide');
    });

    $(document).on('click', '.repairsandkycstatusadd', function() {
        //alert($(this).data('name'));
        /*var modalToggle = document.getElementById($(this).id); // relatedTarget
        //var myModal = document.getElementById($('#roleModal'))
        //myModal.show(modalToggle);
        ...
        $('select[name^="status"] option:selected').attr("selected",null);
        $('select[name^="status"] option[value="'+ $(this).data('status') +'"]').attr("selected","selected");*/
        $('#repairsandkycstatusaddModal').modal('show');
    });
    $(document).on('click', '.closeadd', function() {
        $('#repairsandkycstatusaddModal').modal('hide');
    });
