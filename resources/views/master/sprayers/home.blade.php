    ...
    <div class="w-full overflow-hidden rounded-lg shadow-md">
        <div class="flex flex-wrap items-center px-4 py-2 bg-gray-50 dark:bg-gray-800">
            ...
            <div class="relative w-full max-w-full flex-grow flex-1 text-right">
                <a href="{{route('sprayers.addview')}}"class="action-btn py-1.5 font-light text-sm px-4 inline-block mt-2 rounded-lg text-center shadow-md bg-blue-500 text-white  transition duration-200 sprayersadd" type="button" data-toggle="modal" data-target="#roleModal">Add Sprayer</a>
            </div>
        </div>
        <div class="w-full overflow-x-auto">
            ...
            <input type="text" id="searchInput" onkeyup="filterTable()" placeholder="Search for sprayers..." class="px-4 py-2   bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-md">
        </div>
                <table class="w-full divide-y divide-gray-200 " id="sprayers">
                <thead>
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:divide-gray-700 dark:bg-gray-800">
                    ...
                    @foreach($sprayers as $sprayer)
                        <tr class="text-sm bg-white dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-700 dark:text-gray-400 transition duration-200">
                            ...
                            <td class="px-4 py-3">{{ $sprayer->name }}</td>
                            
                            <td class="px-4 py-3">
                                    <button onclick="openModal('{{ asset('sprayers_images').'/'.$sprayer->image }}')" class="">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                </td>
                            <td class="px-4 py-3">{{ $sprayer->description }}</td>
                            <td class="px-4 py-3">
                                @if($sprayer->status == 1)
                                <span class="text-green-600">Active</span>
                                @elseif($sprayer->status == 0)
                                <span class="text-red-600">Inactive</span>
                                @endif
                            </td>
                            <td class="px-4 py-3">
                                <a href="{{route('sprayers.edit',['spary_id' => $sprayer['id']])}}" type="button" class=" sprayersedit" data-toggle="modal" data-target="#sprayersModal" data-id="{{ $sprayer['id'] }}" data-name="{{ $sprayer['name'] }}" data-description="{{ $sprayer['description'] }}" data-status="{{ $sprayer['status'] }}" data-image="{{ asset('sprayers_images').'/'.$sprayer['image'] }}"><i class="fas fa-edit"></i></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-md-6 text-md-start">Add/Edit Sprayers</div>
                        <div class="col-md-6 text-md-end"><button type="button" class="btn btn-primary sprayersadd"  data-toggle="modal" data-target="#roleModal">Add Sprayer</button></div>
                    </div>
                </div>
                    ...
                    <table width="100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?php echo ++$i; ?></td>
                                <td>{{ $sprayer->name }}</td>
                                <td><img src='{{ asset("sprayers_images")."/".$sprayer->image }}' width="100px" /></td>
                                <td>{{ $sprayer->description }}</td>
                                <td>
                                    @if($sprayer->status == 1)
                                        Active
                                    @elseif($sprayer->status == 0)
                                ...
                                <td><button type="button" class="btn btn-primary sprayersedit"  data-toggle="modal" data-target="#sprayersModal" data-id="{{ $sprayer['id'] }}" data-name="{{ $sprayer['name'] }}" data-description="{{ $sprayer['description'] }}" data-status="{{ $sprayer['status'] }}" data-image="{{ asset('sprayers_images').'/'.$sprayer['image'] }}" >Edit</button></td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
...
<!-- Modal -->
<!-- <div class="modal fade" id="sprayersModal" tabindex="-1" role="dialog" aria-labelledby="sprayersModalCenterTitle" aria-hidden="true" style="z-index: 9999;">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="sprayersModalLongTitle">Edit Sprayer</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{ route('sprayers.update') }}" method="post">
            @csrf
...
<!-- <div class="modal fade" id="sprayersaddModal" tabindex="-1" role="dialog" aria-labelledby="sprayersModalCenterTitle" aria-hidden="true" style="z-index: 9999;">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="sprayersModalLongTitle">Add Sprayers</h5>
        <button type="button" class="closeadd" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form action="{{ route('sprayers.add') }}" method="post" enctype="multipart/form-data">
            @csrf
...
<script type="text/javascript">
    function updateMachine(){
        ...
        if(status == status2){
            ...
            if(id == 1 || id == 2 || id == 3){
                if(confirm('This Sprayer in Sprayers section in the home will be made '+ status_val +'. Do you want to continue?')){
    ...
    $(document).on('click', '.sprayersedit', function() {
        ...
        $('#sprayersModal').modal('show');
    });

    $(document).on('click', '.close', function() {
        $('#sprayersModal').modal('hide');
    });
    $(document).on('click', '.closebutton', function() {
        $('#sprayersModal').modal('hide');
    });

    $(document).on('click', '.sprayersadd', function() {
        //alert($(this).data('name'));
        /*var modalToggle = document.getElementById($(this).id); // relatedTarget
        //var myModal = document.getElementById($('#roleModal'))
        //myModal.show(modalToggle);
        ...
        $('select[name^="status"] option:selected').attr("selected",null);
        $('select[name^="status"] option[value="'+ $(this).data('status') +'"]').attr("selected","selected");*/
        $('#sprayersaddModal').modal('show');
    });
    $(document).on('click', '.closeadd', function() {
        $('#sprayersaddModal').modal('hide');
    });
...
function filterTable() {
    ...
    const table = document.getElementById('sprayers');
